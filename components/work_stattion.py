...
def draw_grid(screen, scroll, scroll_v):
    ...
    for r in range(0, MAX_ROWS //2 + 1):
        ...
        pygame.draw.line(screen, E_ORANGE, (0, y_pos), (WIDTH, y_pos), 1)
...
def enviroment(sur):
    ...
    button_list = CreateButtonList(image_list, (962, 17), 1, 7, 35, 30)
                ...
                for x in range(MAX_COLS):
                    ...
                    if tile is not None:
                        img = pygame.transform.scale(
                            image_list[tile][0],
                            (int(image_list[tile][0].get_width() * 0.25), int(image_list[tile][0].get_height() * 0.25))
                        )
                        sur.blit(img, ((x-MAX_COLS//2) * TILE_SIZE - scroll, (y-MAX_ROWS//2) * TILE_SIZE - scroll_v))
    ...
    while run:
            ...
            if pos[0] < 940 and pos[1] < 546:
                ...
                if 1 <= active_layer <= 10:
                    ...
                    if -MAX_COLS < x < MAX_COLS and -MAX_ROWS < y < MAX_ROWS:
                        ...
                        adjusted_x = x + MAX_COLS//2 if x < 0 else x + MAX_COLS//2
                        adjusted_y = y + MAX_ROWS//2 if y < 0 else y + MAX_ROWS//2
        ...
        pygame.draw.rect(sur, (255, 0, 0), button_list[current_tile].rect, 1)
        if menu_display:
